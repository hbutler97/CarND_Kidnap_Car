...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  for(int k = 0; k < num_particles; k++) {
    Particle particle;
    particle.id = k;
    particle.x = dist_x(rand_number);
    particle.y = dist_y(rand_number);
    particle.theta = dist_theta(rand_number);
    particle.weight = 1.0;
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for(int i = 0; i < num_particles; i++) {
    ...
    particles[i].x += dist_x(rand_number);
    particles[i].y += dist_y(rand_number);
    particles[i].theta += dist_theta(rand_number);
  }
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  for(int i = 0; i < num_observations; i++) {
    ...
    for(int k = 0; k < num_predictions; k++) {
      double x_dist = observations[i].x - predicted[k].x;
      double y_dist = observations[i].y - predicted[k].y;
...
void ParticleFilter::resample() {
  ...
  for(int i = 0; i < num_particles; i++) {
    beta += distDouble(rand_number) * 2.0;
